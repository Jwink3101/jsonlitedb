...
- `readme.md` includes executed demo output injected between `<!--- BEGIN AUTO GENERATED -->` and `<!--- END AUTO GENERATED -->`; that block is overwritten on each run. To modify that text, edit `Demo/Basic Usage.md`.
- `build_help.py` runs `md-demo` on `Demo/Basic Usage.md` and inserts a cleaned copy into the README.
