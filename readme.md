...
For case-insensitive matching or sorting, use `lower_()` to apply SQLite `LOWER()` to the extracted value:

    db.query(db.Q.name.lower_() == "john")
    db.query(db.Q.email.lower_() % "%@example.com")
    db.query(db.Q.name.lower_() * "jo*")
    db.query(_orderby=db.Q.last.lower_())
    db.create_index(db.Q.name.lower_())

You can also use `length_()` to query or sort by the extracted value length:

    db.query(db.Q.name.length_() >= 4)
    db.query(_orderby=db.Q.name.length_())
    db.create_index(db.Q.name.length_())

Use `wrap_()` for other SQLite scalar functions:

    db.query(db.Q.created.wrap_("unixepoch") >= cutoff)
    db.query(db.Q.created.wrap_("substr", 1, 7) == "2025-01")
    db.query(_orderby=db.Q.created.wrap_("unixepoch"))

The extracted value is the first function argument by default. Use
`db.Q.VALUE_` to position it explicitly:

    db.Q.created.wrap_("strftime", "%Y", db.Q.VALUE_)

See [Query Functions](Demo/Query%20Functions.md) for executable examples,
including expression indexes and direct SQLite comparisons when both sides
need SQL functions.
